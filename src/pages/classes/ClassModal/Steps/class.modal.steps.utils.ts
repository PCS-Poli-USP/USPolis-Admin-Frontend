...
export function scheduleToString(schedule: ScheduleData) {
  if (
    ...
    } de ${moment(schedule.start_date).format('DD/MM/YYYY')} até ${moment(
      schedule.end_date,
    ).format('DD/MM/YYYY')} - ${schedule.start_time} ~ ${schedule.end_time}`;
  else if (schedule.recurrence === Recurrence.MONTHLY) {
    return `${Recurrence.translate(
      schedule.recurrence,
    )}, na ${MonthWeek.translate(schedule.month_week as MonthWeek)} ${WeekDay.translate(schedule.week_day as WeekDay)}s de ${moment(
      schedule.start_date,
